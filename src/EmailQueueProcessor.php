...
class EmailQueueProcessor extends BuildTask
{
    ...
    {
        if (!Director::is_cli()) {
            throw new Exception(__METHOD__ . ': This task can only be ran via command line!');
        }
        ...
        if (!$email_messages->exists()) {
            ...
            if ($later_scheduled_messages->exists()) {
                echo '(However, ' . $later_scheduled_messages->count() . ' messages have been scheduled to be sent at a later time).';
            }
